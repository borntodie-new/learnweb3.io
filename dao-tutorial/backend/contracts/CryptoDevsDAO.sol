...
contract CryptoDevsDAO is Ownable {
    ...
    struct Proposal {
        ...
        bool executed;
        // voters - a mapping of CryptoDevsNFT tokenIDs to booleans indicating whether that NFT has alread been used to cast a vote or not
    ...
    // The payable allows this construtor to accept an ETG deposit when it is being deployed
    ...
    modifier inactiveProposalOnly(uint256 proposalIndex) {
        require(
            proposals[proposalIndex].deadline <= block.timestamp,
            "DEADLINE_NOT_EXCEEDES"
        );
        require(
            proposals[proposalIndex].executed == false,
            "PROPOSAL_ALREAD_EXECUTED"
        );
        _;
    }
    ...
    function createProposal(
        ...
        // Set the proposal voting deadline to be (current time  plus 5 minutes)
